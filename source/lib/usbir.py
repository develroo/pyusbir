...
class shutterglass:
  ...
  def set_rate(self,rate=86):
    ...
    if rate==86: 
      self.cmdTimings = [0x01,0x00,0x18,0x00,0x41,0xEC,0xFE,0xFF,0x66,0xB8,0xFF,0xFF,0x30,0xF8,0xFF,0xFF,0x24,0x28,0x30,0x22,0x05,0x08,0x0A,0x04,0x82,0xD8,0xFD,0xFF] #86Hz için
    else:
      self.cmdTimings = [0x01,0x00,0x18,0x00,w&0xFF,w>>8&0xFF,w>>16&0xFF,w>>24&0xFF,x&0xFF,x>>8&0xFF,x>>16&0xFF,x>>24&0xFF,y&0xFF,y>>8&0xFF,y>>16&0xFF,y>>24&0xFF,0x24,0x28,0x30,0x22,0x05,0x08,0x0A,0x04,z&0xFF,z>>8&0xFF,z>>16&0xFF,z>>24&0xFF]
    self.write(self.cmdTimings)
    self.write([0x01,0x1c,0x02,0x00,0x02,0x00])
    self.write([0x01,0x1E,0x02,0x00,0xAA,0x00])
    if rate==86:
      self.write([0x01,0x1b,0x01,0x00,0x07])
    else:
      self.write([0x01,0x1b,0x01,0x00,int(rate)*2,int(rate*2)>>8&256])
      #self.write([0x01,0x1b,0x01,0x00,0x07])
    self.write([0x40,0x18,0x03,0x00])
    print self.cap, 'New values sent!'
    print self.cap, 'NVIDIA stereo controller is set to ', rate
    return True
  ...
  def set_eye(self,eye=0,r=0):
    if self.mode3d == 1:
      ...
      #self.write([0xAA,0xFE,0x00,0x00,0xAC,0x1E,0xFF,0xFF])
